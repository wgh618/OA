...
<!--显示表单内容-->
<div id=MainArea>
    <form action="${pageContext.request.contextPath}/role/setPrivilege" method="post">
                        ...
                        <td>
                            <ul id='tree'>
                                <c:forEach items="${privilegeList}" var="topPrivilege">
                                    <li id='li_${topPrivilege.id}'>
                                        <input type='checkbox' name='resourceIdList' id='cb_${topPrivilege.id}'
                                               onclick='doChecked("li_${topPrivilege.id}", this.checked)'
                                               ${oa:isAssignedPrivilege(topPrivilege, rolePrivilegeList)} value="${topPrivilege.id}"/>
                                        <label for='cb_${topPrivilege.id}'><span class='folder'
                                                                                 id='${topPrivilege.id}'>
                                                ${topPrivilege.name}</span></label>
                                        <ul>
                                            <c:forEach items="${topPrivilege.children}" var="secendPrivilege">
                                                <li id='li_${secendPrivilege.id}'>
                                                    <input type='checkbox' name='resourceIdList' value="${secendPrivilege.id}"
                                                           id='cb_${secendPrivilege.id}'
                                                           onclick='doChecked("li_${secendPrivilege.id}",
                                                                   this.checked)' ${oa:isAssignedPrivilege(secendPrivilege, rolePrivilegeList)}/>
                                                    <label for='cb_${secendPrivilege.id}'><span class='folder'
                                                                                                id='${secendPrivilege.id}'>${secendPrivilege.name}</span></label>
                                                    <ul>
                                                        <c:forEach items="${secendPrivilege.children}"
                                                                   var="lastPrivilege">
                                                            <li id='li_${lastPrivilege.id}'>
                                                                <input type='checkbox' name='resourceIdList' value="${lastPrivilege.id}"
                                                                       id='cb_${lastPrivilege.id}'
                                                                       onclick='doChecked("li_${lastPrivilege.id}", 
                                                                               this.checked)' ${oa:isAssignedPrivilege(lastPrivilege, rolePrivilegeList)}/>
                                                                <label for='cb_${lastPrivilege.id}'><span class='folder'
                                                                                                          id='${lastPrivilege.id}'>
                                                                        ${lastPrivilege.name}
                                                                </span></label>
                                                            </li>
