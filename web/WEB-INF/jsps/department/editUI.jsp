...
<!--显示表单内容-->
<div id=MainArea>
    <form action="${pageContext.request.contextPath}/department/edit" method="post" onsubmit="return check();">
                ...
                <table cellpadding="0" cellspacing="0" class="mainForm">
                    <tr><td width="100">上级部门</td>
                        <td><select name="parentId" class="SelectStyle">
                                ...
                                <c:forEach var="department1" items="${departmentList}">
                                    <c:choose>
                                        <c:when test="${department1.id eq department.parent.id}">
                                            <option value="${department1.id}" selected>${department1.name}</option>
                                        </c:when>
                                        <c:otherwise>
